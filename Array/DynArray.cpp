...
template <>
DArray<std::string>::DArray(std::string *d, size_t size1) {
    if (this->size  == 0)
        this->data = (std::string *)calloc (size1, sizeof (std::string));
    else
        this->data = (std::string *)realloc (this->data, size1 * sizeof (std::string));
    for (size_t i = 0; i < size1; i++)
        *(this->data + i) = *(d + i);
    this->size = size1;
}
